            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {overdue.data.overdue.map((row) => (
                  <tr key={row.invoiceId} className="border-t border-neutral-100">
                    <td className="py-2 font-medium">{row.invoiceNo}</td>
                    <td className="py-2 text-neutral-600">{row.contactName}</td>
...
function ReminderRow({
  ...
  const tone =
    reminder.tone === 'OWNER_ALERT' ? 'bg-red-50' : reminder.tone === 'FIRM' ? 'bg-amber-50' : 'bg-neutral-50';
  ...
  return (
    <div className={`rounded-md p-3 ${tone}`}>
      ...
      <p className="mt-1 whitespace-pre-wrap text-sm text-neutral-700">{reminder.message}</p>
      {canSend ? (
