...
| Testing | **Vitest** unit, a **real PostgreSQL 16** for integration tests (`scripts/pg-dev.sh` locally, a GitHub Actions service container in CI — revised from Testcontainers during implementation, because Docker is not available in every environment this is developed in and a harness that cannot start is a harness nobody runs), **fast-check** property tests on the ledger | Property tests for double-entry: for any generated set of transactions, debits equal credits, the trial balance balances, and reversing every entry returns all balances to zero. This is the cheapest possible insurance against the class of bug that destroys the product's credibility. |
...
| CI/CD | **GitHub Actions** → build, test, scan, push to ECR, deploy | Pipeline gates: typecheck → lint → unit → integration (real PostgreSQL service container) → property tests on the ledger → build → SAST (CodeQL) → dependency audit → container scan (Trivy) → IaC scan (tfsec) → secret scan (gitleaks). A failing ledger property test blocks the merge, no override. |
