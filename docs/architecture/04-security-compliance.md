...
- **Hash-chained:** `row_hash = SHA256(prev_hash || canonical_json(row))`. A nightly job verifies the chain and alarms on a break. This does not prevent tampering by someone with database superuser access — it makes it *detectable*, which is the achievable goal. Ship the daily chain head to append-only storage (S3 Object Lock) for an independent anchor.
